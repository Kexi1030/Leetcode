...
```sql
select de.dept_name,ifnull(student_number,0) as student_number
from (
    select count(*) as student_number,dept_id
    from student
    group by dept_id
    ) temp right join department as de
on de.dept_id = temp.dept_id
order by student_number desc ,de.dept_name asc
```

**首先按照dept_id进行分类 从student中找出每个dept的学生个数和dept_id 再用右连接与departentment表格连接 按照题目要求对结果的学生人数降序排列 对部门名字升序排列 注意如果stundnet_num为null的时候 将其置为0**
